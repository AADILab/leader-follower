...
class BoidSpawner():
    def __init__(self,
        ...
        if self.fix_velocities:
            self.fixed_velocities = self.generateVelocities()

        if self.fix_headings:
            self.fixed_headings = self.generateHeadings()

        self.fixed_is_leader = self.generateIsLeader()
    ...
    def generateVelocities(self) -> NDArray[np.float64]:
        if self.velocity_rule.value == VelocityRule.UniformRandom.value:
            ...
            velocity = self.velocity_fraction*(self.bounds.max_velocity - self.bounds.min_velocity) + self.bounds.min_velocity
    ...
    def generateIsLeader(self) -> NDArray[np.bool_]:
        # Leaders, Followers
        return np.array( [True for _ in range(self.bounds.num_leaders)] + [False for _ in range(self.bounds.num_followers)] )

    def getSpawnState(self):
        if self.fix_positions:
        ...
        if self.fix_velocities:
            ...
            velocities = self.generateVelocities()
