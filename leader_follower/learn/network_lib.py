...
class NN:
    def __init__(self, num_inputs: int, num_hidden: Union[int, List[int]], num_outputs: int) -> None:
        ...
        self.weights_shape = calculate_weight_shape(self.weights)
        self.total_weights = calculate_weight_size(self.weights)
        return

    def random_weights(self) -> List[np.ndarray]:
        weights = []
    ...
    def set_weights(self, weights: List[np.ndarray]):
        ...
        if any(dimensions_incorrect):
            raise Exception(f'Weights are being set incorrectly in setWeights().\n'
                            f'Dimensions for new weights and network weights do not match!\n'
                            f'New weights dim != Network weights dim\n'
                            f'{[w.shape for w in weights]} != {[w.shape for w in self.weights]}'
                            )
        # Set weights
    ...
    @staticmethod
    def activation(arr: np.ndarray) -> np.ndarray:
        return np.tanh(arr)
...
def create_nn_from_weights(weights: List[np.ndarray]):
    weight_shape = calculate_weight_shape(weights)
...
def calculate_weight_shape(weights: List[np.ndarray]):
    return tuple([w.shape for w in weights])


def calculate_weight_size(weights: List[np.ndarray]) -> Tuple[Tuple[int]]:
