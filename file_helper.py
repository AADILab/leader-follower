...
def loadTrial(trialname: str)->Dict:
    return pickle.load(open(trialname+".pkl", "rb"))
...
def getLatestTrialNum()->int:
    filenames = [f for f in listdir(".") if isfile(f) and f[-4:] == ".pkl" and f[:6] == "trial_"]
...
def saveTrial(save_data: Dict, trial_num: Optional[str]=None)->None:
    if trial_num is None:
        ...
        trial_name = "trial_"+trial_num+".pkl"
    pickle.dump(save_data, open(trial_name, "wb"))
