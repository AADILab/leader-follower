import sys; sys.path.append("/Users/sidd/Desktop/Summer REU/PBRS_DFollow_plus_DRiP/leader-follower")
...
def main():
    ...
    trial_num = 49
    num_stat_runs = 20
    computer_name = "drip_twocoupling"

    tested_FCoupleD = True
    tested_FCouple = True
    tested_G = True
    tested_Dfollow = True
    ...
    if tested_FCouple:
        ...
        for i in range(num_stat_runs):
            ...
            trial_num-=1
        print("FCoupleD trials: ", trials_FCouple)
        print()
    ...
    if tested_FCoupleD: 
        avg_FCD, std_dev_FCD, upper_dev_FCD, lower_dev_FCD, upper_range_FCD, lower_range_FCD = getStatistics(trials_FCoupleD, computer_name)
        num_generations_arr = np.arange(avg_FCD.shape[0])
        plt.plot(num_generations_arr, avg_FCD, color='tab:orange')
        legend.append("$FCoupleD$")
    
    if tested_FCouple: 
        avg_D, std_dev_D, upper_dev_D, lower_dev_D, upper_range_D, lower_range_D = getStatistics(trials_FCouple, computer_name)
    ...
    if tested_drip_FCouple:
        avg_Z, std_dev_Z, upper_dev_Z, lower_dev_Z, upper_range_Z, lower_range_Z = getStatistics(trials_drip_FCouple, computer_name)
        num_generations_arr = np.arange(avg_Z.shape[0])
        plt.plot(num_generations_arr, avg_Z, color="tab:pink")
        legend.append(r"$DRiP_{FCouple}$")
    
    if tested_drip_FCoupleD: 
        avg_Dr_FCD, std_dev_FCD, upper_dev_FCD, lower_dev_FCD, upper_range_FCD, lower_range_FCD = getStatistics(trials_drip_FCoupleD, computer_name)
        num_generations_arr = np.arange(avg_Dr_FCD.shape[0])
        plt.plot(num_generations_arr, avg_Dr_FCD, color="tab:red")
        legend.append(r'$DRiP_{FCoupleD}$')
    ...
    if tested_drip_FCouple:
        plt.fill_between(num_generations_arr, upper_dev_FCD, lower_dev_FCD, alpha=0.2, color="tab:red")
        if plot_min_max_range:
            plt.fill_between(num_generations_arr, upper_range_FCD, lower_range_FCD, alpha=0.2, color="tab:red")
    ...
    plot_save_name = "figures/driptest_20statrun_nominmax.png"
